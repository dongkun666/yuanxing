...
> 自动生成于 2026-06-29T13:32:07Z, 来自 PRD backlog open P0+P1 ticket → mavis team plan YAML

## 状态

- **Status**: DRY-RUN (no mavis plan queued)
- **调度命令**: `mavis team plan run --no-wait E:\元枢法智前端\yuanxing\docs\plans\plan-auto-w10-from-backlog.yaml`
- **Plan YAML**: [plan-auto-w10-from-backlog.yaml](./plan-auto-w10-from-backlog.yaml) (full path: `E:\元枢法智前端\yuanxing\docs\plans\plan-auto-w10-from-backlog.yaml`)
- **触发**: scripts/auto_schedule_backlog.py (W9 A2)

## 数据源

- **Database**: `prd_backlog` (SQLite)
- **过滤条件**: `status='open' AND priority IN ('P0', 'P1') `
- **Ticket 总数**: 140
...
| --- | ---: | --- | --- |
| 产品 | 60 | lex-ai, lex-coder, lex-design | P0=32, P1=28 |
| 技术 | 28 | lex-ai, lex-coder | P0=12, P1=16 |
| 法务 | 40 | lex-ai | P0=20, P1=20 |
| 其他 | 12 | lex-pm | P0=4, P1=8 |
...
version: 1
plan:
  name: 'LexPrime Phase 4 Week 10 自动调度 (from PRD backlog open P0/P1, 140 ticket)'
  max_concurrency: 2
  ...
  metadata:
    ...
    generated_at: '2026-06-29T13:32:07Z'
...
tasks:
  ...
  # Task auto: 产品 backlog 调度 (60 ticket)
  - id: 'product-w10-auto'
    title: 'W10 产品 backlog 自动调度 (60 ticket: P0=32, P1=28)'
    prompt: |
      W10 A2 自动调度: 从 PRD backlog 拉取 产品 类 open P0/P1 ticket (60 个), assigned_to=lex-pm. 
...
```bash
mavis team plan run --no-wait E:\元枢法智前端\yuanxing\docs\plans\plan-auto-w10-from-backlog.yaml
```
...
2. Owner (Mavis/lex-pm) 验收: 启动 `mavis team plan run E:\元枢法智前端\yuanxing\docs\plans\plan-auto-w10-from-backlog.yaml` 真启 plan (本期 A2 默认 dry-run, 不污染 owner 队列)
