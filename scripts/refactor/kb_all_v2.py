...
# Step 4: data-* 属性 (用更稳的 regex - 把整个 <tr ...> 完整匹配)
# 改用：找到 data-knowledge-area="XXX"> 这个完整 token, 在 " 之前 插入新属性
# 这样不会破坏原有属性
# 用 lookbehind: 找 data-knowledge-area="XXX" 后面的 > 字符
# 但 Python re 不支持 lookbehind with variable length in some cases
# 用更直接的: replace ` data-knowledge-area="XXX">` with ` data-knowledge-area="XXX" data-date="..." data-cite="..." data-title="...">`
# 但要先知道 XXX 的值...

# 更简单: 用 regex 匹配整个 tr 开头 tag (含 >)
pattern = re.compile(
    r'<tr class="hover:bg-\[#F7F8FA\] transition-colors" data-knowledge-type="[^"]*" data-knowledge-area="[^"]*">'
)
matches = list(pattern.finditer(raw))
print(f'找到 {len(matches)} 个数据行')

def process_match(m):
    start, end = m.start(), m.end()
    # m.group(0) 是完整 tr tag 含 >
    # 提取这个 row body (到下一个 </tr>)
    body_start = end
    body_end = raw.find('</tr>', body_start)
    body = raw[body_start:body_end]
    # 从 body 提取 data
    title_match = re.search(r'<p class="text-(?:xs|sm) font-medium text-\[#1D2129\][^"]*">([^<]+)</p>', body)
    title = title_match.group(1) if title_match else ''
    date_match = re.search(r'(\d{4}-\d{2}-\d{2})', body)
    date = date_match.group(1) if date_match else ''
    cite_match = re.search(r'<td class="text-center py-3 px-4 text-xs font-medium text-\[#165DFF\]">(\d+)</td>', body)
    cite = cite_match.group(1) if cite_match else '0'
    # 替换 end-1 位置 (>) 前面加新属性
    old_tag = m.group(0)
    new_tag = old_tag[:-1] + f' data-date="{date}" data-cite="{cite}" data-title="{title}">'
    return start, end, new_tag

# 倒序替换 (从后往前 不影响前面 match index)
new_raw = raw
for m in reversed(matches):
    start, end, new_tag = process_match(m)
    new_raw = new_raw[:start] + new_tag + new_raw[end:]

# 验证
print('data-date:', new_raw.count('data-date="'))
print('data-cite:', new_raw.count('data-cite="'))
print('data-title:', new_raw.count('data-title="'))
