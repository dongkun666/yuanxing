        ...
        calDates.forEach(function (cell) {
            ...
            if (cellDay === dayNum) {
                ...
                if (!cell.querySelector('.court-schedule-marker')) {
                    ...
                    courtMarker.className =
                        'court-schedule-marker text-[8px] text-red-600 font-medium block leading-none mt-0.5';
    ...
    Object.keys(dateGroups).forEach(function (dateKey) {
        ...
        calDates.forEach(function (cell) {
            ...
            if (cellDay === dayNum) {
                ...
                conflictMarker.className =
                    'schedule-conflict-red w-1.5 h-1.5 rounded-full bg-red-500 inline-block mx-auto mt-0.5';
                // 移除原有指示点（蓝色/红色），只标记冲突
...
function checkAndShowConflict() {
    ...
    if (conflicts && conflicts.length > 0) {
        detailEl.innerHTML = conflicts
            .map(
                (c) =>
                    '• <span class="font-medium">' +
                    c.title +
                    '</span><br><span class="text-amber-600">' +
                    c.time +
                    ' - ' +
                    c.endTime +
                    '</span>'
...
function toggleScheduleComplete(id) {
    ...
    // 详情页如果开着也刷新 (避免状态不一致)
    if (
        typeof currentDetailScheduleId !== 'undefined' &&
        currentDetailScheduleId === id &&
        typeof openScheduleDetail === 'function'
    ) {
...
function renderScheduleCheckbox(s) {
    ...
    var btnCls = done ? 'bg-brand border-brand text-white' : 'border-bg-border hover:border-brand bg-white';
    var icon = done ? '<iconify-icon icon="mdi:check-bold" class="text-xs"></iconify-icon>' : '';
    return (
        '<button type="button" onclick="event.stopPropagation(); toggleScheduleComplete(' +
        s.id +
        ')" ' +
        'class="w-4 h-4 rounded border-2 ' +
        btnCls +
        ' flex items-center justify-center transition-colors flex-none" ' +
        'title="' +
        (done ? '已完成 (点击取消)' : '标记为完成') +
        '">' +
        icon +
...
function renderScheduleFilterTabs(containerId, scope) {
    ...
    opts.forEach(function (o) {
        ...
        var cls = active ? 'bg-brand text-white' : 'bg-bg-subtle text-fg-secondary hover:bg-bg';
        htmlStr +=
...
function persistSchedule() {
    try {
        localStorage.setItem('lexprime_schedule_data', JSON.stringify(AppState.scheduleData));
    } catch (e) {
        console.warn('持久化日程失败:', e);
    }
}
...
function renderScheduleList() {
    ...
    if (!container) return;
    var empty = document.getElementById('schedule-empty') || document.getElementById('schedule-empty-state');
    var countEl = document.getElementById('schedule-count');

    // 排序: 未完成优先 (completed 沉底), 同状态按 date+time 升序
    var sorted = AppState.scheduleData.filter(getScheduleFilterPredicate('list')).sort(function (a, b) {
    ...
    if (sorted.length === 0) {
        ...
        if (empty) {
            if (typeof Utils !== 'undefined' && Utils.renderEmptyState) {
                empty.innerHTML = Utils.renderEmptyState({
                    type: 'default',
                    icon: 'mdi:calendar-check-outline',
                    title: '暂无日程安排',
                    description: '还没有创建任何日程，点击下方按钮开始您的第一个日程安排',
                    actionText: '新建日程',
                    actionHandler: openScheduleModal
                });
            }
    ...
    sorted.forEach(function (s) {
        ...
        var done = !!s.completed;
        var titleCls = done
            ? 'text-sm font-medium text-fg-tertiary line-through'
            : 'text-sm font-medium text-fg-primary';

        htmlStr +=
            '<div class="group ' +
            (done ? 'bg-bg-subtle ' : 'bg-white ') +
            'rounded-xl border border-bg-border p-4 hover:shadow-sm transition-shadow border-l-4 cursor-pointer ' +
            c.border +
            '" data-date="' +
            (s.date || '') +
            '" data-year="' +
            ...
            '" data-month="' +
            (dateObj ? String(dateObj.getMonth() + 1).padStart(2, '0') : '') +
            '" data-day="' +
            (dateObj ? String(dateObj.getDate()).padStart(2, '0') : '') +
            '" data-schedule-id="' +
            s.id +
            ...
            '</span>' +
            (isToday
                ? '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-urgent text-white font-medium">今天</span>'
                : '') +
            '</div>' +
            '<div class="text-xs text-fg-tertiary flex items-center gap-3 flex-wrap">' +
            '<span><iconify-icon class="text-xs" icon="mdi:clock-time-four-outline"></iconify-icon> ' +
            (s.time || '') +
            (s.endTime ? ' - ' + s.endTime : '') +
            '</span>' +
            (s.location
                ? '<span><iconify-icon class="text-xs" icon="mdi:map-marker-outline"></iconify-icon> ' +
            ...
            '<button class="text-sm text-fg-secondary hover:bg-bg-subtle px-2.5 py-1 rounded flex items-center gap-1" onclick="event.stopPropagation(); openScheduleDetail(' +
            s.id +
            ')" title="详情">' +
            '<iconify-icon icon="mdi:eye-outline"></iconify-icon>' +
            '<span>详情</span>' +
            '</button>' +
            '<button class="text-sm text-brand hover:bg-brand-tint3 px-2.5 py-1 rounded flex items-center gap-1" onclick="event.stopPropagation(); openScheduleModal(' +
            s.id +
            ')" title="修改">' +
            '<iconify-icon icon="mdi:pencil-outline"></iconify-icon>' +
            '<span>修改</span>' +
            '</button>' +
            '<button class="text-sm text-red-500 hover:bg-red-50 px-2.5 py-1 rounded flex items-center gap-1" onclick="event.stopPropagation(); deleteScheduleItem(' +
            s.id +
            ')" title="删除">' +
...
function goToCaseFromDetail() {
    ...
    var item = AppState.scheduleData.find(function (s) {
        return s.id === currentDetailScheduleId;
    });
    if (!item || !item.caseId) {
        showToast('该日程未关联案件');
        return;
...
function openAITrialCoach() {
    ...
    var script = [
        {
            role: '法官',
            text: '现在开庭。请原告陈述诉讼请求与理由。',
            ai: '提示: 简明扼要 (3 点以内), 围绕"案由 + 诉请金额 + 主要事实", 不超过 2 分钟。'
        },
        {
            role: '原告律师',
            text: '（你的回应）',
            ai: '提示: 先说请求, 再展开事实和法律依据。最后引用 1-2 个最高法指导案例。'
        },
        {
            role: '法官',
            text: '请被告答辩。',
            ai: '提示: 针对原告诉请逐条反驳, 强调"程序违法/事实不清/证据不足" 至少一点。'
        },
        { role: '被告律师', text: '（你的回应）', ai: '提示: 不要重复原告, 主动提出反诉或管辖异议 (如适用)。' },
        {
            role: '法官',
            text: '进入举证质证环节。',
            ai: '提示: 围绕 3-5 个关键证据, 按"三性" (真实性/合法性/关联性) 质证。'
        },
        {
            role: '原告律师',
            text: '（提交证据）',
            ai: '提示: 按时间线/因果链展示, 关键证据 (合同/付款凭证) 用 PPT 高亮。'
        },
        {
            role: '被告律师',
            text: '（质证意见）',
            ai: '提示: 对真实性无异议的不必纠缠, 集中攻击关键证据的"三性"缺陷。'
        },
        {
            role: '法官',
            text: '法庭辩论结束, 双方最后陈述。',
            ai: '提示: 30 秒内, 再次强调核心诉请, 表达调解意向 (如可能)。'
        },
        { role: '原告律师', text: '（最后陈述）', ai: '提示: 简明, 不重复事实, 强调法律适用, 表达对法庭的尊重。' },
        {
            role: '法官',
            text: '现在休庭, 双方阅卷笔录签字。',
            ai: 'AI 复盘: 控制发言时间 (每人 5-8 分钟), 注重礼仪, 关键证据 PPT 提前演练, 应对突发问题 (如新证据) 准备 "申请延期举证"。'
        }
    ];
    ...
    var roleColor = {
        法官: 'bg-purple-100 text-purple-700',
        原告律师: 'bg-blue-100 text-blue-700',
        被告律师: 'bg-red-100 text-red-700'
    };
    modal.innerHTML =
        '<div class="bg-white rounded-xl w-[720px] max-h-[85vh] flex flex-col shadow-2xl" onclick="event.stopPropagation()">' +
        '<div class="flex items-center justify-between p-4 border-b border-bg-border">' +
        '<div><h3 class="text-base font-semibold text-fg-primary">AI 庭审模拟 · ' +
        escapeHtml(caseName) +
        '</h3>' +
        ...
        script
            .map(function (s, i) {
                ...
                return (
                    '<div class="flex gap-3"><div class="flex-shrink-0 w-20 text-xs font-semibold ' +
                    colorCls +
                    ' px-2 py-1 rounded text-center h-fit">' +
                    s.role +
                    '</div>' +
                    '<div class="flex-1"><p class="text-sm text-fg-primary mb-1.5">' +
                    escapeHtml(s.text) +
                    '</p>' +
                    '<div class="bg-amber-50 border-l-2 border-amber-400 p-2 rounded-r"><p class="text-xs text-amber-800"><span class="font-semibold">AI 提示: </span>' +
                    escapeHtml(s.ai) +
                    '</p></div></div></div>'
                );
            })
...
function renderTodaySchedule() {
    ...
    if (items.length === 0) {
        ...
        if (emptyEl) {
            ...
            var todayAll = AppState.scheduleData.filter(function (s) {
                return s.date === today;
            });
            var todayDone = todayAll.filter(function (s) {
                return s.completed;
            }).length;
            if (typeof Utils !== 'undefined' && Utils.renderEmptyState) {
                ...
                if (f === 'pending' && todayDone > 0) {
                    ...
                    emptyEl.innerHTML =
                        ...
                        '<p class="text-[10px] text-fg-tertiary mt-0.5 mb-2">' +
                        todayDone +
                        ' 项已完成</p>' +
    ...
    displayItems.forEach(function (s) {
        ...
        var done = !!s.completed;
        var titleCls = done
            ? 'text-sm font-medium text-fg-tertiary line-through truncate'
            : 'text-sm font-medium text-fg-primary truncate';
        htmlStr +=
            '<div class="group flex items-start gap-2 p-2 rounded-lg ' +
            (done ? 'bg-bg-subtle ' : '') +
            'hover:bg-brand-tint3 transition-colors cursor-pointer" data-schedule-id="' +
            s.id +
            '">' +
            ...
            (isPast ? 'text-fg-tertiary line-through' : c.text) +
            '">' +
            (s.time || '--:--') +
            '</span>' +
            '</div>' +
            ...
            escapeHtml(s.location || s.caseName || s.type || '') +
            '</p>' +
            '</div>' +
            '<div class="flex-none ' +
            c.iconColor +
            ...
            '<button class="w-6 h-6 rounded hover:bg-brand-tint flex items-center justify-center text-fg-tertiary hover:text-brand transition-colors" onclick="event.stopPropagation(); openScheduleDetail(' +
            s.id +
            ')" title="详情">' +
            '<iconify-icon class="text-sm" icon="mdi:eye-outline"></iconify-icon>' +
            '</button>' +
            '<button class="w-6 h-6 rounded hover:bg-brand-tint flex items-center justify-center text-fg-tertiary hover:text-brand transition-colors" onclick="event.stopPropagation(); openScheduleModal(' +
            s.id +
            ')" title="修改">' +
            '<iconify-icon class="text-sm" icon="mdi:pencil"></iconify-icon>' +
            '</button>' +
            '<button class="w-6 h-6 rounded hover:bg-red-100 flex items-center justify-center text-fg-tertiary hover:text-red-500 transition-colors" onclick="event.stopPropagation(); deleteScheduleItem(' +
            s.id +
            ')" title="删除">' +
    ...
    if (overflowCount > 0) {
        htmlStr +=
            '<div class="text-center pt-2 border-t border-bg-border mt-1">' +
...
function initScheduleFilterToToday() {
    ...
    if (
        yearEl &&
        !Array.from(yearEl.options).some(function (o) {
            return o.value === y;
        })
    ) {
        var opt = document.createElement('option');
    ...
    if (
        dayEl &&
        !Array.from(dayEl.options).some(function (o) {
            return o.value === d;
        })
    ) {
...
function openScheduleDetail(id) {
    ...
    const typeColors = {
        开庭: ['bg-purple-100', 'text-purple-700'],
        会议: ['bg-blue-100', 'text-blue-700'],
        待办: ['bg-green-100', 'text-green-700'],
...
function checkDetailConflict(item) {
    ...
    const conflicts = AppState.scheduleData.filter(
        (s) => s.id !== item.id && s.date === item.date && item.time < s.endTime && item.endTime > s.time
    );

    if (conflicts.length > 0) {
        detailEl.innerHTML = conflicts
            .map((c) => '• <span class="font-medium">' + c.title + '</span> (' + c.time + '-' + (c.endTime || '') + ')')
            .join('<br>');
        warningEl.classList.remove('hidden');
    } else {
        warningEl.classList.add('hidden');
...
function buildConflictResolveContent(conflicts) {
    ...
    var busyPeriods = conflicts.map(function (c) {
        return { start: c.time, end: c.endTime };
    });
    var suggestions = suggestFreeSlots(date, busyPeriods);
    var slotsHtml = suggestions
        .map(function (s) {
            return (
    ...
    return (
        ...
        '<div class="space-y-2" id="conflict-list">' +
        listHtml +
        '</div>' +
        ...
        '</div>' +
        '<div class="mt-2 flex flex-wrap gap-2" id="suggested-slots">' +
        slotsHtml +
        '</div>' +
        '</div>' +
        '</div>'
...
function suggestFreeSlots(date, busyPeriods) {
    ...
    return allSlots
        .filter((slot) => !busyPeriods.some((busy) => slot.start < busy.end && slot.end > busy.start))
        .slice(0, 4);
}
...
function checkCourtConflicts() {
    ...
    if (conflicts.length > 0) {
        detail.innerHTML = conflicts
            .map(
                (c) =>
                    '• <span class="font-medium">' +
                    c.a.title +
                    '</span> 与 <span class="font-medium">' +
                    c.b.title +
                    '</span> 时间重叠（' +
                    c.a.date +
                    ' ' +
...
function openCaseDynamicDetail(index) {
    var dynamics = [
        {
            ...
            handler: '李明',
            description:
                '张三合同纠纷一案的举证期限将于2026年6月23日截止，请尽快整理并提交相关证据材料，避免因逾期导致证据失权。',
            files: ['证据目录_v3.xlsx (256KB)', '举证期限告知书.pdf (1.2MB)'],
            note: '请务必在截止日前完成证据交换，已通知对方代理人。',
            action: '去处理'
        },
        {
            type: '文书',
            ...
            handler: '王芳',
            description: '李四借贷纠纷案的民事起诉状已完成最终审核，经合伙人确认无误，可安排打印盖章后提交法院立案。',
            files: ['民事起诉状_终稿.docx (45KB)'],
            note: '已安排下周一早提交立案庭。',
            action: '查看文书'
        },
        {
            ...
            handler: '赵磊',
            description: '根据最新补充的银行流水和股权变更登记材料，已更新证据目录，新增证据5-8号，请确认是否完整。',
            files: ['证据目录_更新版.xlsx (128KB)', '补充材料_银行流水.pdf (3.5MB)'],
            note: '',
            action: '查看详情'
        },
        {
            ...
            handler: '陈静',
            description:
                '赵六诉某科技公司劳动争议案，经与法院沟通，开庭时间定于2026年7月15日上午9:00，在市劳动争议仲裁委员会第一仲裁庭。',
            files: ['开庭传票.pdf (0.5MB)'],
            note: '请提前30分钟到达，带齐证据原件。',
            action: '查看详情'
        },
        {
            type: '开庭',
            ...
            handler: '刘强',
            description:
                '孙七建设工程合同纠纷案合议庭成员已确定，审判长：张明法官，审判员：李华、王丽。当事人对合议庭成员如申请回避，需在5日内提出。',
            files: ['合议庭组成通知书.pdf (0.3MB)'],
            note: '已与当事人确认无回避申请。',
            action: '查看详情'
        },
        {
            ...
            handler: '李明',
            description:
                '周八借款纠纷案已结案归档。判决已生效，案卷材料已按档案管理规定整理完毕，存放于档案室第3柜第12号。',
            files: ['结案报告.docx (32KB)', '判决书.pdf (0.8MB)'],
            note: '归档编号：2026-0312',
            action: '查看归档'
        },
        {
            ...
            handler: '王芳',
            description:
                '吴九房屋租赁合同纠纷案一审判决书已收到并上传系统。判决结果：被告支付租金及违约金合计￥45,600。双方是否上诉待确认。',
            files: ['一审判决书.pdf (1.1MB)'],
            note: '已通知当事人查收判决书，上诉期限15天。',
            action: '查看判决书'
        },
        {
            type: '提醒',
            ...
            title: '续约提醒',
            caseName: '常年法律顾问 - 某科技公司',
            time: '2026-06-03 10:00',
            handler: '系统自动',
            description: '某科技公司常年法律顾问服务合同将于2026年7月1日到期，如需续约请提前30天联系客户沟通续约事宜。',
            files: ['顾问合同_2025.pdf (0.6MB)'],
            note: '客户满意度较高，建议主动联系续约。',
            action: '查看详情'
        }
    ];
    var d = dynamics[index] || dynamics[0];
    document.getElementById('detail-type-badge').textContent = d.type;
    document.getElementById('detail-type-badge').className =
        'text-[10px] font-medium px-1.5 py-0.5 rounded-full ' + d.typeClass;
    ...
    if (filesContainer) {
        ...
        (d.files || []).forEach(function (f) {
            ...
            filesHtml +=
                '<div class="flex items-center gap-3 bg-gray-50 rounded-lg px-3 py-2.5 hover:bg-gray-100 transition-colors cursor-pointer">' +
...
function removeDynamicFile(fileId) {
    AppState.dynamicAttachments = AppState.dynamicAttachments.filter(function (f) {
        return f.id !== fileId;
    });
    renderDynamicFilePreview();
}
...
function renderDynamicFilePreview() {
    ...
    AppState.dynamicAttachments.forEach(function (f) {
        html +=
            ...
            '</p>' +
            '</div>' +
            '<button onclick="removeDynamicFile(\'' +
            f.id +
...
function submitNewDynamic() {
    ...
    if (AppState.dynamicAttachments.length > 0) {
        attachHtml =
            ...
            AppState.dynamicAttachments
                .map(function (a) {
                    return (
                        '<span class="inline-flex items-center gap-1 text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-full"><iconify-icon icon="' +
                        a.icon +
    ...
    var cardHtml =
        '<div class="bg-white rounded-xl border border-[#E5E6EB] p-4 hover:shadow-sm transition-shadow ' +
        colors.border +
        '">' +
        ...
        (isUrgent ? '紧急' : AppState.selectedDynamicType) +
        '</span>' +
        '<span class="font-medium text-sm text-gray-800">' +
        escapeHtml(title) +
        '</span>' +
        '</div>' +
        '<p class="text-xs text-gray-500">案件：' +
        escapeHtml(caseName) +
        ' · ' +
        (escapeHtml(desc.substring(0, 30)) || '暂无详细描述') +
        '</p>' +
        attachHtml +
        '<div class="flex items-center gap-3 mt-2">' +
        '<span class="text-[10px] text-gray-400"><iconify-icon icon="mdi:clock-outline" class="mr-0.5"></iconify-icon>' +
        timeStr +
        '</span>' +
    ...
    document.querySelectorAll('.dyn-type-option').forEach(function (b, i) {
        b.className =
            'dyn-type-option text-xs px-3 py-1.5 rounded-full ' +
            (i === 0 ? 'bg-[#165DFF] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200');
    });
    // 重置附件
    AppState.dynamicAttachments = [];
    renderDynamicFilePreview();
...
function switchDynamicsView(view) {
    ...
    if (view === 'timeline') {
        listBtn.className =
            'flex items-center gap-1 h-8 px-2.5 text-xs bg-white text-gray-600 hover:bg-gray-50 transition-colors';
...
function renderDynamicsTimeline() {
    ...
    AppState.dynamicsViewData.forEach(function (d, i) {
        ...
        htmlStr +=
            '<div class="dyn-tl-item relative pb-6">' +
            '<div class="absolute left-[-22px] top-1 w-3 h-3 rounded-full border-2 bg-white ' +
            (dotColor || 'border-blue-500') +
            '"></div>' +
            '<div class="bg-white rounded-xl border border-[#E5E6EB] p-4 hover:shadow-sm transition-shadow cursor-pointer" onclick="openCaseDynamicDetail(' +
            i +
            ')">' +
            '<div class="flex items-start gap-3">' +
            '<div class="w-8 h-8 rounded-lg ' +
            d.typeClass.split(' ')[0].replace('text-', 'bg-').replace('-700', '-50') +
            ' flex items-center justify-center flex-shrink-0">' +
            '<iconify-icon icon="' +
            d.icon +
            '" class="' +
