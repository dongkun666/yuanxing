...
(function () {
    ...
    function renderJudicialList() {
        ...
        var html =
            ...
            '<input type="text" id="judicial-search-input" placeholder="搜索观点标题/全文/法条/案例号..." class="w-full bg-bg-subtle border border-bg-border rounded-lg pl-9 pr-3 py-2 text-sm text-fg-primary focus:outline-none focus:border-brand focus:bg-white transition-colors" value="' +
            escapeHtml(currentSearchKw) +
            '" oninput="onJudicialSearch(this.value)"/>' +
            '</div>' +
            ...
            '<option value="all"' +
            (currentCauseFilter === 'all' ? ' selected' : '') +
            '>全部案由</option>' +
            causes
                .map(function (c) {
                    return (
            ...
            '<option value="date"' +
            (currentSortBy === 'date' ? ' selected' : '') +
            '>按时间</option>' +
            '<option value="cause"' +
            (currentSortBy === 'cause' ? ' selected' : '') +
            '>按案由</option>' +
            '<option value="source"' +
            (currentSortBy === 'source' ? ' selected' : '') +
            '>按来源</option>' +
            '</select>' +
            '</div>' +
            '<div class="flex items-center justify-between text-[10px] text-fg-tertiary">' +
            '<span><iconify-icon class="text-xs" icon="mdi:format-list-bulleted"></iconify-icon> 共 ' +
            points.length +
            ' 条观点' +
            (currentSearchKw || currentCauseFilter !== 'all' ? ' (已筛选)' : '') +
            '</span>' +
            '<span><iconify-icon class="text-xs" icon="mdi:star-outline"></iconify-icon> 已收藏 ' +
            Object.keys(_favorites).length +
            ' 条</span>' +
            '</div>' +
            '</div>' +
            // 列表
    ...
    function renderPointCard(p) {
        ...
        return (
            '<div class="bg-white rounded-xl border border-bg-border p-4 hover:shadow-md hover:border-brand/30 transition-all cursor-pointer" onclick="openJudicialPoint(\'' +
            p.id +
            '\')">' +
            '<div class="flex items-start justify-between gap-3 mb-2">' +
            '<div class="flex items-center gap-2 flex-wrap flex-1 min-w-0">' +
            '<h3 class="text-sm font-semibold text-fg-primary hover:text-brand transition-colors">' +
            escapeHtml(p.title) +
            '</h3>' +
            '<span class="text-[10px] ' +
            causeCls +
            ' px-1.5 py-0.5 rounded-full font-medium whitespace-nowrap">' +
            escapeHtml(p.cause) +
            '</span>' +
            '</div>' +
            '<button class="text-fg-tertiary hover:text-amber-500 transition-colors flex-shrink-0" onclick="event.stopPropagation(); toggleJudicialFavorite(\'' +
            p.id +
            '\')" title="' +
            (isFav ? '取消收藏' : '收藏') +
            '">' +
            ...
            '</button>' +
            '</div>' +
            '<p class="text-xs text-fg-secondary leading-relaxed line-clamp-2 mb-2">' +
            escapeHtml(p.summary) +
            '</p>' +
            '<div class="flex items-center gap-3 text-[10px] text-fg-tertiary flex-wrap">' +
            '<span class="inline-flex items-center gap-0.5"><iconify-icon class="text-xs" icon="mdi:account-outline"></iconify-icon> ' +
            escapeHtml(p.author) +
            '</span>' +
            '<span class="text-fg-disabled">·</span>' +
            '<span class="inline-flex items-center gap-0.5"><iconify-icon class="text-xs" icon="mdi:calendar-outline"></iconify-icon> ' +
            escapeHtml(p.date) +
            '</span>' +
            '<span class="text-fg-disabled">·</span>' +
            '<span class="inline-flex items-center gap-0.5"><iconify-icon class="text-xs" icon="mdi:bookshelf"></iconify-icon> ' +
            escapeHtml(p.source) +
            '</span>' +
            ((p.tags || []).length > 0
                ? '<span class="text-fg-disabled">·</span>' +
                  ...
                  p.tags
                      ...
                      .map(function (t) {
                          return (
                              '<span class="px-1.5 py-0.5 rounded bg-bg-subtle text-fg-secondary">#' +
                              escapeHtml(t) +
                              '</span>'
                          );
                      })
    ...
    function renderJudicialDetail(p) {
        ...
        var fullTextHtml = (p.fullText || '')
            ...
            .map(function (line) {
                ...
                if (/^【.+】$/.test(trimmed)) {
                    return (
                        '<h3 class="text-sm font-semibold text-fg-primary mt-4 mb-2">' + escapeHtml(trimmed) + '</h3>'
                    );
                }
        ...
        var html =
            ...
            '</button>' +
            '<button class="text-xs px-3 py-1.5 rounded-lg border flex items-center gap-1 transition-colors ' +
            (isFav
                ? 'border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100'
                : 'border-bg-border text-fg-secondary hover:bg-bg-subtle') +
            '" onclick="toggleJudicialFavorite(\'' +
            p.id +
            '\')">' +
            '<iconify-icon class="text-base" icon="' +
            (isFav ? 'mdi:star' : 'mdi:star-outline') +
            ...
            (p.tags || [])
                ...
                .map(function (t) {
                    return (
                        '<span class="text-[10px] px-2 py-0.5 rounded-full bg-bg-subtle text-fg-secondary">#' +
                        escapeHtml(t) +
                        '</span>'
                    );
                })
                .join('') +
            '</div>' +
            '<h1 class="text-xl font-bold text-fg-primary mb-3 leading-tight">' +
            escapeHtml(p.title) +
            '</h1>' +
            '<p class="text-sm text-fg-secondary leading-relaxed mb-3">' +
            escapeHtml(p.summary) +
            '</p>' +
            '<div class="flex items-center gap-3 text-[11px] text-fg-tertiary flex-wrap">' +
            '<span class="inline-flex items-center gap-1"><iconify-icon class="text-xs" icon="mdi:account-outline"></iconify-icon> ' +
            escapeHtml(p.author) +
            '</span>' +
            '<span class="text-fg-disabled">·</span>' +
            '<span class="inline-flex items-center gap-1"><iconify-icon class="text-xs" icon="mdi:bookshelf"></iconify-icon> ' +
            escapeHtml(p.source) +
            '</span>' +
            '<span class="text-fg-disabled">·</span>' +
            '<span class="inline-flex items-center gap-1"><iconify-icon class="text-xs" icon="mdi:calendar-outline"></iconify-icon> ' +
            escapeHtml(p.date) +
            '</span>' +
            '</div>' +
            '</div>' +
            ...
            ((p.applicableScenarios || []).length > 0
                ? '<div class="bg-white rounded-xl border border-bg-border p-5">' +
                  ...
                  p.applicableScenarios
                      .map(function (s) {
                          return (
                              '<div class="flex items-start gap-2 text-xs"><iconify-icon class="text-sm text-success flex-shrink-0 mt-0.5" icon="mdi:check-circle"></iconify-icon> <span class="text-fg-secondary">' +
                              escapeHtml(s) +
                              '</span></div>'
                          );
                      })
            ...
            ((p.relatedLaws || []).length > 0
                ? '<div class="bg-white rounded-xl border border-bg-border p-5">' +
                  ...
                  p.relatedLaws
                      .map(function (l) {
                          return (
                              '<div class="flex items-start gap-2 text-xs"><iconify-icon class="text-sm text-brand flex-shrink-0 mt-0.5" icon="mdi:bookmark-outline"></iconify-icon> <span class="text-fg-secondary font-mono">' +
                              escapeHtml(l) +
                              '</span></div>'
                          );
                      })
            ...
            ((p.citedCases || []).length > 0
                ? '<div class="bg-white rounded-xl border border-bg-border p-5">' +
                  ...
                  p.citedCases
                      .map(function (c) {
                          return (
                              '<div class="flex items-start gap-2 text-xs"><iconify-icon class="text-sm text-urgent flex-shrink-0 mt-0.5" icon="mdi:file-document-outline"></iconify-icon> <span class="text-fg-secondary font-mono">' +
                              escapeHtml(c) +
                              '</span></div>'
                          );
                      })
