...
(function () {
    ...
    function getStatusBadge(status) {
        ...
        return (
            '<span class="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full flex-shrink-0">' +
            escapeHtml(status) +
            '</span>'
        );
    }
    ...
    function renderCompanyCard(company) {
        ...
        if (_manageMode) {
            manageBtn =
                '<button class="text-[10px] text-red-500 hover:text-red-600 hover:underline flex-shrink-0 ml-3" onclick="event.stopPropagation(); unfollowCompany(' +
                company.id +
                ')">' +
        ...
        return (
            '<div class="p-5 hover:bg-bg-subtle transition-colors cursor-pointer" onclick="openCompanyDetail(' +
            company.id +
            ')">' +
            ...
            '<iconify-icon class="text-2xl ' +
            company.iconColor +
            '" icon="mdi:domain"></iconify-icon>' +
            '</div>' +
            ...
            '<div><span class="text-fg-disabled">法定代表人：</span><span class="text-fg-secondary">' +
            escapeHtml(company.legalRep) +
            '</span></div>' +
            '<div><span class="text-fg-disabled">注册资本：</span><span class="text-fg-secondary">' +
            escapeHtml(company.registeredCapital) +
            '</span></div>' +
            '<div><span class="text-fg-disabled">成立日期：</span><span class="text-fg-secondary">' +
            escapeHtml(company.establishDate) +
            '</span></div>' +
            '</div>' +
            '<div class="flex items-center gap-4 mt-2 text-[10px]">' +
            '<span class="text-red-500"><iconify-icon class="inline" icon="mdi:alert-circle-outline"></iconify-icon> 法律风险 ' +
            company.legalRisk +
            '</span>' +
            '<span class="text-amber-500"><iconify-icon class="inline" icon="mdi:alert-outline"></iconify-icon> 经营风险 ' +
            company.operatingRisk +
            '</span>' +
            '<span class="text-blue-500"><iconify-icon class="inline" icon="mdi:information-outline"></iconify-icon> 知识产权 ' +
            company.ipCount +
            '</span>' +
            manageBtn +
            '</div>' +
    ...
    function renderEmpty(message) {
        ...
        container.innerHTML =
            ...
            '<p class="text-sm text-fg-tertiary mt-3">' +
            escapeHtml(msg) +
            '</p>' +
            '</div>';
    }
