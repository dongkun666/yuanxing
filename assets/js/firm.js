(function() {
    ...
    function renderMemberList() {
        ...
        container.innerHTML = list.map(function(member, idx) {
            ...
            return '<div class="flex items-center gap-3 p-2 rounded-lg hover:bg-bg-subtle transition-colors cursor-pointer" onclick="openMemberDetail(' + member.id + ')">' +
    ...
    function openFirmSetting(key) {
        ...
        tabs.forEach(function(t) {
            var active = t === key;
            tabsHtml += '<button class="flex-1 min-w-[70px] text-[11px] py-1.5 px-2 rounded-md transition-colors ' +
                (active ? 'bg-white text-fg-primary shadow-sm font-medium' : 'text-fg-tertiary hover:text-fg-secondary') +
                '" onclick="switchFirmSettingTab(\'' + t + '\')">' + _settingLabels[t] + '</button>';
        });
        tabsHtml += '</div>';
    ...
    function switchFirmSettingTab(key) {
        ...
        if (modal) {
            ...
            if (titleEl) titleEl.innerHTML = '<iconify-icon icon="mdi:cog-outline" class="text-brand text-lg"></iconify-icon>律所设置 - ' + escapeHtml(label);
        }
        ...
        tabs.forEach(function(btn) {
            ...
            if (tabKey === key) {
                btn.className = 'flex-1 min-w-[70px] text-[11px] py-1.5 px-2 rounded-md transition-colors bg-white text-fg-primary shadow-sm font-medium';
            } else {
                btn.className = 'flex-1 min-w-[70px] text-[11px] py-1.5 px-2 rounded-md transition-colors text-fg-tertiary hover:text-fg-secondary';
            }
        });
    }
    ...
    function renderFirmSettingContent(key) {
        ...
        if (key === 'members') {
            return '<div class="space-y-3">' +
                ...
                '<p class="text-xs text-fg-tertiary">共 ' + _membersData.length + ' 名成员</p>' +
                '<button class="text-[11px] text-brand hover:underline">+ 添加成员</button>' +
                '</div>' +
                '<div class="space-y-2">' +
                _membersData.map(function(m, idx) {
                    var color = _avatarColors[idx % _avatarColors.length];
                    return '<div class="flex items-center gap-3 p-2 rounded-lg hover:bg-bg-subtle">' +
                        '<div class="w-9 h-9 rounded-full ' + color.bg + ' flex items-center justify-center flex-shrink-0">' +
                        '<span class="text-xs font-semibold ' + color.text + '">' + escapeHtml(m.name.charAt(0)) + '</span>' +
                        '</div>' +
                        '<div class="flex-1 min-w-0">' +
                        '<p class="text-xs font-medium text-fg-primary">' + escapeHtml(m.name) + '</p>' +
                        '<p class="text-[10px] text-fg-tertiary">' + escapeHtml(m.position) + ' · ' + escapeHtml(m.field) + '</p>' +
                        '</div>' +
        ...
        if (key === 'departments') {
            ...
            return '<div class="space-y-2">' +
                depts.map(function(d) {
                    return '<div class="flex items-center justify-between p-3 bg-white border border-bg-border rounded-lg">' +
                        '<div>' +
                        '<p class="text-xs font-medium text-fg-primary">' + escapeHtml(d.name) + '</p>' +
                        '<p class="text-[10px] text-fg-tertiary mt-0.5">负责人: ' + escapeHtml(d.head) + '</p>' +
                        '</div>' +
                        '<span class="text-[11px] text-fg-secondary bg-bg-subtle px-2 py-0.5 rounded">' + d.count + ' 人</span>' +
                        '</div>';
                }).join('') +
                '</div>';
        }
        if (key === 'fees') {
            ...
            return '<div class="space-y-2">' +
                fees.map(function(f) {
                    return '<div class="flex items-center justify-between p-3 bg-white border border-bg-border rounded-lg">' +
                        '<span class="text-xs text-fg-primary">' + escapeHtml(f.type) + '</span>' +
                        '<span class="text-xs font-semibold text-brand">' + escapeHtml(f.rate) + '</span>' +
                        '</div>';
                }).join('') +
        ...
        if (key === 'contracts') {
            ...
            return '<div class="grid grid-cols-2 gap-2">' +
                contracts.map(function(c) {
                    return '<div class="p-3 bg-white border border-bg-border rounded-lg">' +
                        '<div class="flex items-center gap-2 mb-1">' +
                        '<iconify-icon icon="' + c.icon + '" class="text-brand"></iconify-icon>' +
                        '<span class="text-xs font-medium text-fg-primary">' + escapeHtml(c.name) + '</span>' +
                        '</div>' +
                        '<p class="text-[10px] text-fg-tertiary">累计使用 ' + c.count + ' 次</p>' +
                        '</div>';
                }).join('') +
                '</div>';
        }
        if (key === 'permissions') {
            ...
            return '<div class="space-y-2">' +
                roles.map(function(r) {
                    return '<div class="flex items-center justify-between p-3 bg-white border border-bg-border rounded-lg">' +
                        '<div>' +
                        '<span class="text-[10px] ' + r.color + ' font-medium px-2 py-0.5 rounded-full">' + escapeHtml(r.name) + '</span>' +
                        '<p class="text-[11px] text-fg-tertiary mt-1">' + escapeHtml(r.desc) + '</p>' +
                        '</div>' +
    ...
    function openMemberDetail(id) {
        ...
        var content = '<div class="space-y-4">' +
            ...
            '</div>' +
            '<div class="flex-1 min-w-0">' +
            '<h3 class="text-base font-semibold text-fg-primary">' + escapeHtml(member.name) + '</h3>' +
            '<p class="text-xs text-fg-tertiary mb-1">' + escapeHtml(member.position) + '</p>' +
            '<span class="text-[10px] ' + (member.online ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-500') + ' font-medium px-2 py-0.5 rounded-full">' + (member.online ? '在线' : '离线') + '</span>' +
            '</div>' +
            '</div>' +
            '<div class="grid grid-cols-2 gap-3 text-sm">' +
            '<div class="p-3 bg-white border border-bg-border rounded-lg">' +
            '<p class="text-[11px] text-fg-tertiary mb-1">执业领域</p>' +
            '<p class="text-sm text-fg-primary font-medium">' + escapeHtml(member.field) + '</p>' +
            '</div>' +
            '<div class="p-3 bg-white border border-bg-border rounded-lg">' +
            '<p class="text-[11px] text-fg-tertiary mb-1">入职时间</p>' +
            '<p class="text-sm text-fg-primary font-medium">20' + (15 + (id % 10)) + '-' + String((id * 3) % 12 + 1).padStart(2, '0') + '</p>' +
            '</div>' +
            '</div>' +
            '<div class="p-3 bg-bg-subtle rounded-xl space-y-2">' +
            '<div class="flex items-center justify-between text-sm">' +
            '<span class="text-fg-tertiary">执业证号</span>' +
            '<span class="text-fg-primary font-mono text-xs">110' + (100000 + id * 12345) + '</span>' +
            '</div>' +
            '<div class="flex items-center justify-between text-sm">' +
            '<span class="text-fg-tertiary">联系电话</span>' +
            '<span class="text-fg-primary text-xs">138****' + String(1000 + id * 137).slice(-4) + '</span>' +
            '</div>' +
            '<div class="flex items-center justify-between text-sm">' +
