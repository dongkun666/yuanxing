...
(function () {
        ...
        fetch(SA.apiBase + '/contracts')
            ...
            .catch(function (err) {
                ...
                SA.contracts = [
                    ...
                    {
                        contract_id: 'demo-service-tech-2026',
                        contract_type: '服务合同',
                        contract_title: '上海市某 IT 服务采购合同 (2026)',
                        icon: 'mdi:laptop',
                        risk_count: '6 条款 / 1fatal+0major+4advisory',
                        amount: '面议',
                        stance: '乙方'
                    },
                    {
                        contract_id: 'demo-sales-goods-2026',
                        contract_type: '销售合同',
                        contract_title: '广东省某货物销售合同 (2026)',
        ...
        tabs.innerHTML = SA.contracts
            .map(function (c) {
                ...
                var activeClass = active
                    ? colorMap[c.contract_type] || 'bg-fg-primary text-white border-fg-primary'
                ...
                return (
                    ...
                    '">' +
                    '<iconify-icon icon="' +
                    esc(c.icon || 'mdi:file-document-outline') +
                    '" class="text-sm align-middle mr-0.5"></iconify-icon>' +
                    '<span>' +
                    esc(c.contract_type) +
                    '</span></button>'
                );
    ...
    function selectContract(contractId) {
        var c = SA.contracts.find(function (x) {
    ...
    function renderCurrentContractInfo() {
        ...
        if (preview)
            preview.textContent =
                c.contract_text || '（合同全文从后端 contract_review/fixtures 接口获取, 此处仅显示摘要）';
    }
    ...
    function renderDimensions() {
        ...
        container.innerHTML = DIMENSIONS.map(function (d, idx) {
            ...
            return (
                ...
                '      <span class="w-5 h-5 rounded-full bg-' +
                d.color +
                '-tint text-' +
                d.color +
                ...
                '</span>' +
                '    </label>' +
                '    <p class="text-[11px] text-fg-tertiary mt-0.5 leading-relaxed">' +
                esc(d.description) +
                '</p>' +
                '    <p class="text-[10px] text-fg-tertiary mt-1 italic">' +
                esc(d.hint) +
                '</p>' +
                '  </div>' +
                '  <div class="flex flex-col items-end flex-shrink-0">' +
                '    <span class="text-3xl font-bold score-number ' +
                colorMap[d.color] +
                '" data-dim-display="' +
                esc(d.id) +
                '">5</span>' +
                ...
                '    class="score-slider flex-1" data-dim-slider="' +
                esc(d.id) +
                '" ' +
                '    aria-label="' +
                esc(d.label) +
    ...
    function showStatus(type, msg) {
        ...
        status.innerHTML =
            '<iconify-icon icon="' + iconMap[type] + '" class="text-sm align-middle"></iconify-icon> ' + esc(msg);
    }
    ...
    function saveCache() {
        try {
            localStorage.setItem('w6_score_cache_v1', JSON.stringify(SA.submittedCache));
        } catch (e) {}
    }

    function saveHistory() {
        try {
            localStorage.setItem('w6_score_history_v1', JSON.stringify(SA.history));
        } catch (e) {}
    }
    ...
    function renderHistory() {
        ...
        list.innerHTML = SA.history
            .slice()
            .reverse()
            .map(function (h) {
                var dt = new Date(h.submitted_at);
                var dtStr = dt.toLocaleString('zh-CN', { hour12: false });
                var cachedTag = h.cached
                    ? '<span class="text-[9px] px-1 py-0.5 bg-warning-tint text-warning rounded-sm ml-1">已暂存</span>'
                    : '';
                var scoreColor =
                    h.aggregate_score >= 7 ? 'text-success' : h.aggregate_score >= 4 ? 'text-warning' : 'text-danger';
                return (
                    '<div class="flex items-center justify-between gap-2 px-3 py-2 bg-bg-subtle rounded-md border border-bg-border">' +
                    ...
                    esc(dtStr) +
                    ' · ' +
                    esc(h.contract_title || h.contract_id || '') +
                    '</div>' +
                    '  </div>' +
                    '</div>' +
