...
(function() {
    ...
    var LAWS = [
        // ----- 法律 (全国人大常委会) -----
        { id: 'law-001', name: '中华人民共和国民法典', publishInfo: '2020年5月28日第十三届全国人民代表大会第三次会议通过 · 2021年1月1日施行', level: '法律', organ: '全国人大常委会', status: '现行有效' },
        { id: 'law-002', name: '中华人民共和国民事诉讼法', publishInfo: '2023年9月1日修正 · 2024年1月1日施行', level: '法律', organ: '全国人大常委会', status: '现行有效' },
        { id: 'law-003', name: '中华人民共和国劳动合同法', publishInfo: '2012年12月28日修正', level: '法律', organ: '全国人大常委会', status: '现行有效' },
        { id: 'law-004', name: '中华人民共和国公司法', publishInfo: '2023年12月29日修订 · 2024年7月1日施行', level: '法律', organ: '全国人大常委会', status: '现行有效' },
        { id: 'law-005', name: '中华人民共和国刑法', publishInfo: '1997年3月14日修订 · 历经十二次修正', level: '法律', organ: '全国人大常委会', status: '现行有效' },
        { id: 'law-006', name: '中华人民共和国经济合同法', publishInfo: '1981年12月13日第五届全国人大第四次会议通过', level: '法律', organ: '全国人大常委会', status: '已废止' },

        // ----- 行政法规 (国务院) -----
        { id: 'law-007', name: '中华人民共和国劳动合同法实施条例', publishInfo: '2008年9月18日国务院令第535号公布', level: '行政法规', organ: '国务院', status: '现行有效' },
        { id: 'law-008', name: '女职工劳动保护特别规定', publishInfo: '2012年4月28日国务院令第619号公布', level: '行政法规', organ: '国务院', status: '现行有效' },
        { id: 'law-009', name: '物业管理条例', publishInfo: '2007年8月26日修订 · 国务院令第504号', level: '行政法规', organ: '国务院', status: '现行有效' },

        // ----- 司法解释 (最高人民法院 / 最高人民检察院) -----
        { id: 'law-010', name: '最高人民法院关于审理民间借贷案件适用法律若干问题的规定', publishInfo: '法释〔2020〕17号 · 2021年1月1日施行', level: '司法解释', organ: '最高人民法院', status: '现行有效' },
        { id: 'law-011', name: '最高人民法院关于适用《中华人民共和国民法典》合同编通则若干问题的解释', publishInfo: '法释〔2023〕13号 · 2023年12月5日施行', level: '司法解释', organ: '最高人民法院', status: '现行有效' },
        { id: 'law-012', name: '最高人民法院关于审理劳动争议案件适用法律问题的解释（一）', publishInfo: '法释〔2020〕26号 · 2021年1月1日施行', level: '司法解释', organ: '最高人民法院', status: '现行有效' },
        { id: 'law-013', name: '最高人民检察院关于人民检察院直接受理立案侦查案件立案标准的规定（试行）', publishInfo: '高检发研字〔1999〕10号', level: '司法解释', organ: '最高人民检察院', status: '已废止' },

        // ----- 部门规章 (各部委) -----
        { id: 'law-014', name: '公安机关办理刑事案件程序规定', publishInfo: '2020年7月20日公安部令第159号', level: '部门规章', organ: '公安部', status: '现行有效' },
        { id: 'law-015', name: '律师事务所管理办法', publishInfo: '2018年9月18日司法部令第133号', level: '部门规章', organ: '司法部', status: '现行有效' },
        { id: 'law-016', name: '劳动人事争议仲裁办案规则', publishInfo: '2017年5月8日人社部令第33号', level: '部门规章', organ: '人社部', status: '现行有效' },
    ...
    function renderLaws(list) {
        ...
        container.innerHTML = list.map(function(law, idx) {
            var levelBadge = LEVEL_STYLES[law.level] || 'bg-gray-50 text-gray-600';
            var statusHtml = law.status === '已废止'
                ? '<span class="text-gray-400"> · 已废止</span>'
                : '<span class="text-fg-tertiary"> · 现行有效</span>';
            return '' +
                '<div class="flex items-center gap-3 p-4 hover:bg-bg-subtle transition-colors cursor-pointer" onclick="openLawDetail(\'' + law.id + '\')">' +
                    '<span class="text-xs font-bold text-brand w-6">' + (idx + 1) + '</span>' +
                    '<div class="flex-1 min-w-0">' +
                        '<h3 class="text-sm font-medium text-fg-primary hover:text-brand transition-colors truncate">' + escapeHtml(law.name) + '</h3>' +
                        '<p class="text-[10px] text-fg-tertiary mt-0.5 truncate">' + escapeHtml(law.publishInfo) + statusHtml + '</p>' +
                    '</div>' +
                    '<span class="text-[10px] ' + levelBadge + ' px-2 py-0.5 rounded-full flex-shrink-0">' + law.level + '</span>' +
                '</div>';
        }).join('');
    }
    ...
    function openLawDetail(id) {
        ...
        modal.innerHTML = '<div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">' +
            ...
            '<span class="text-[10px] ' + levelBadge + ' font-medium px-2 py-0.5 rounded-full">' + escapeHtml(law.level) + '</span>' +
            '<span class="text-[10px] ' + statusCls + ' font-medium px-2 py-0.5 rounded-full">' + escapeHtml(law.status) + '</span>' +
            '</div>' +
            '</div>' +
            '<div class="grid grid-cols-2 gap-3 text-sm">' +
            '<div class="p-3 bg-white border border-bg-border rounded-lg">' +
            '<p class="text-[11px] text-fg-tertiary mb-1">制定机关</p>' +
            '<p class="text-sm text-fg-primary font-medium">' + escapeHtml(law.organ) + '</p>' +
            '</div>' +
            '<div class="p-3 bg-white border border-bg-border rounded-lg">' +
            '<p class="text-[11px] text-fg-tertiary mb-1">效力层级</p>' +
            '<p class="text-sm text-fg-primary">' + escapeHtml(law.level) + '</p>' +
            '</div>' +
            '</div>' +
            '<div class="p-3 bg-bg-subtle rounded-xl">' +
            '<p class="text-[11px] text-fg-tertiary mb-1">发布信息</p>' +
            '<p class="text-xs text-fg-secondary leading-relaxed">' + escapeHtml(law.publishInfo) + '</p>' +
            '</div>' +
            '<div class="p-3 bg-bg-subtle rounded-xl">' +
