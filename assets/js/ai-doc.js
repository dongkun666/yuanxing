...
(function() {
    ...
    var DOC_TYPES = {
        'complaint': {
            ...
            fields: [
                ...
                { key: 'claims', label: '诉讼请求', placeholder: '1. 判令被告支付借款本金 XX 元...', type: 'textarea', rows: 4 },
                { key: 'facts', label: '事实与理由', placeholder: '原被告于 2024 年 X 月 X 日签订...', type: 'textarea', rows: 5 }
            ]
        },
        'defense': {
            name: '民事答辩状',
            ...
            fields: [
                ...
                { key: 'court', label: '受理法院', placeholder: '如: 北京市海淀区人民法院', type: 'text' },
                { key: 'response', label: '答辩事项', placeholder: '请求法院依法驳回原告的全部诉讼请求...', type: 'textarea', rows: 4 },
                { key: 'facts', label: '事实与理由', placeholder: '针对原告诉讼请求, 答辩如下...', type: 'textarea', rows: 5 }
            ]
        },
        'lawyer-letter': {
            name: '律师函',
            ...
            fields: [
                ...
                { key: 'background', label: '背景说明', placeholder: '委托人 X 与贵司于 2024 年 X 月 X 日签订...', type: 'textarea', rows: 4 },
                { key: 'demand', label: '要求事项', placeholder: '1. 立即支付欠款 XX 元; 2. 三日内联系...', type: 'textarea', rows: 4 },
        ...
        'contract': {
            ...
            icon: 'mdi:file-sign',
            fields: [
                { key: 'contract_type', label: '合同类型', placeholder: '如: 服务合同/买卖合同/借款合同', type: 'text' },
                { key: 'partyA', label: '甲方', placeholder: '姓名/名称', type: 'text' },
                { key: 'partyB', label: '乙方', placeholder: '姓名/名称', type: 'text' },
                { key: 'subject', label: '标的/服务内容', placeholder: '甲方向乙方提供 XX 服务...', type: 'textarea', rows: 3 },
                { key: 'amount', label: '金额', placeholder: '如: 100,000 元', type: 'text' },
                { key: 'deadline', label: '履行期限', placeholder: '如: 自合同签订之日起 30 日内', type: 'text' },
                { key: 'key_terms', label: '关键条款', placeholder: '违约责任/争议解决/保密等特殊要求', type: 'textarea', rows: 3 }
            ]
        },
        'mediation': {
            name: '调解协议',
            ...
            fields: [
                ...
                { key: 'agreement', label: '调解方案', placeholder: '1. 乙方于 X 日内支付 XX 元; 2. 双方再无其他争议...', type: 'textarea', rows: 4 },
        ...
        'demand-letter': {
            ...
            fields: [
                ...
                { key: 'consequence', label: '后果说明', placeholder: '逾期将通过法律途径追偿, 包括违约金、诉讼费、律师费等', type: 'textarea', rows: 3 }
            ]
        }
    };
    ...
    function renderFormFields(type) {
        ...
        docDef.fields.forEach(function(field) {
            ...
            if (field.type === 'textarea') {
                inputHtml = '<textarea id="ai-doc-field-' + field.key + '" placeholder="' + escapeHtml(field.placeholder) + '" rows="' + (field.rows || 3) + '" class="w-full bg-bg-subtle border border-bg-border rounded-lg px-3 py-2 text-sm text-fg-primary focus:outline-none focus:border-brand focus:bg-white transition-colors resize-none"></textarea>';
            } else {
                inputHtml = '<input type="text" id="ai-doc-field-' + field.key + '" placeholder="' + escapeHtml(field.placeholder) + '" class="w-full bg-bg-subtle border border-bg-border rounded-lg px-3 py-2 text-sm text-fg-primary focus:outline-none focus:border-brand focus:bg-white transition-colors"/>';
            }
            html +=
                '<div>' +
                '<label class="block text-xs font-medium text-fg-secondary mb-1">' + escapeHtml(field.label) + '</label>' +
                inputHtml +
                '</div>';
        });
    ...
    function generateAIDocument() {
        ...
        // 简单校验: 至少填一个字段
        var hasAny = Object.values(data).some(function(v) { return v && v.length > 0; });
        if (!hasAny) {
            showToast('请至少填写一项内容');
            return;
        }
    ...
    function generateLawyerLetter(d, date) {
        ...
        return '律 师 函\n\n' +
            ...
            demand + '\n\n' +
            '四、回复期限\n' +
            '请贵司在 ' + deadline + ' 以书面形式回复本律师, 逾期未回复, 委托人将依法采取进一步法律行动, 包括但不限于提起诉讼/仲裁。\n\n' +
            '特此函告。\n\n' +
            '________ 律师事务所\n' +
            '承办律师: ________\n' +
            '            ' + date + '\n';
    ...
    function generateContract(d, date) {
        ...
        return type.toUpperCase() + '合 同\n\n' +
            '甲方: ' + partyA + '\n' +
            '乙方: ' + partyB + '\n\n' +
            '鉴于: 双方就 ' + subject + ' 经友好协商, 达成如下协议:\n\n' +
            '第一条 标的\n' + subject + '\n\n' +
            '第二条 金额\n本合同总金额为人民币 ' + amount + ' 元。\n\n' +
            '第三条 履行期限\n' + deadline + '\n\n' +
            '第四条 双方权利义务\n4.1 甲方应按时支付合同款项。\n4.2 乙方应按约定提供产品/服务。\n\n' +
    ...
    function generateDemandLetter(d, date) {
        ...
        return '催 告 函\n\n' +
            '致: ' + recipient + '\n\n' +
            '我方与贵方就 ' + subject + ' 一事, 达成如下约定: 贵方应于 ' + dueDate + ' 之前支付 ' + amount + ' 元。\n\n' +
            '截至本函发出之日, 贵方仍未履行上述付款义务, 已构成违约。\n\n' +
            '现正式函告如下:\n' +
            '一、请贵方在收到本函 ' + deadline + ' 内付清上述款项 ' + amount + ' 元。\n' +
            '二、若贵方逾期未付, 我方将采取以下措施:\n' + consequence + '\n\n' +
            '特此函告。\n\n' +
            '签发人: ________\n' +
            '        ' + date + '\n';
