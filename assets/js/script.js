    ...
    function openAddSkillModal() {
        ...
        document.getElementById('skill-category').value = '法律文书';
        document.getElementById('skill-enabled').checked = true;
        selectSkillColor('blue');
    ...
    function editSkill(id) {
        ...
        // 实际应用中这里应该从数据中获取技能信息
        var card = document.querySelector('[data-skill-id="' + id + '"]');
        if (card) {
            var name = card.querySelector('h4').textContent;
            var desc = card.querySelector('.text-xs.text-gray-500').textContent;
            document.getElementById('skill-name').value = name;
            document.getElementById('skill-desc').value = desc;
        }
        document.getElementById('skill-modal').classList.remove('hidden');
    }
    ...
    function selectSkillColor(color) {
        currentSkillColor = color;
        document.querySelectorAll('.skill-color-btn').forEach(function(btn) {
            if (btn.dataset.color === color) {
                btn.classList.add('border-[#165DFF]');
                btn.classList.remove('border-transparent');
            } else {
                btn.classList.remove('border-[#165DFF]');
    ...
    function submitSkill() {
        ...
        var colorMap = {
            'blue': { bg: 'bg-blue-50', text: 'text-blue-600', icon: '#165DFF' },
            'green': { bg: 'bg-green-50', text: 'text-green-600', icon: 'green' },
            'purple': { bg: 'bg-purple-50', text: 'text-purple-600', icon: 'purple' },
            'orange': { bg: 'bg-orange-50', text: 'text-orange-500', icon: 'orange' },
            'red': { bg: 'bg-red-50', text: 'text-red-500', icon: 'red' },
            'cyan': { bg: 'bg-cyan-50', text: 'text-cyan-500', icon: 'cyan' }
        };

        var colors = colorMap[currentSkillColor] || colorMap['blue'];

        var skillId = currentSkillId || (Date.now());
        var list = document.getElementById('skills-list');

        if (currentSkillId) {
            // 编辑现有技能
            var card = document.querySelector('[data-skill-id="' + currentSkillId + '"]');
            if (card) {
                card.querySelector('h4').textContent = name;
                card.querySelector('.text-xs.text-gray-500').textContent = desc;
                card.querySelector('.text-\\[10px\\]').textContent = category;
            }
        } else {
            // 添加新技能
            var newCard = document.createElement('div');
            newCard.className = 'bg-white rounded-xl border border-[#E5E6EB] p-5 skill-card' + (enabled ? '' : ' opacity-60');
            newCard.dataset.skillId = skillId;
            newCard.innerHTML = '<div class="flex items-start justify-between">' +
                '<div class="flex items-start gap-4">' +
                '<div class="w-12 h-12 rounded-xl ' + colors.bg + ' flex items-center justify-center flex-shrink-0">' +
                '<iconify-icon class="text-2xl" style="color: ' + colors.icon + '" icon="mdi:lightning-bolt"></iconify-icon>' +
                '</div>' +
                '<div>' +
                '<h4 class="font-bold text-sm text-[#1D2129]">' + name + '</h4>' +
                '<p class="text-xs text-gray-500 mt-1">' + desc + '</p>' +
                '<div class="flex items-center gap-3 mt-2">' +
                '<span class="text-[10px] ' + colors.bg + ' ' + colors.text + ' px-2 py-0.5 rounded">' + category + '</span>' +
                '<span class="text-[10px] text-gray-400">使用 0 次</span>' +
                '</div>' +
                '</div>' +
                '</div>' +
                '<div class="flex items-center gap-3">' +
                '<label class="relative inline-flex items-center cursor-pointer">' +
                '<input class="sr-only peer" type="checkbox"' + (enabled ? ' checked' : '') + ' onchange="toggleSkill(' + skillId + ', this.checked)"/>' +
                '<div class="w-9 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#165DFF]/20 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[\'\'] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-200 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#165DFF]"></div>' +
                '</label>' +
                '<button class="text-xs text-gray-400 hover:text-[#165DFF] px-2 py-1" onclick="editSkill(' + skillId + ')">编辑</button>' +
                '<button class="text-xs text-gray-400 hover:text-red-500 px-2 py-1" onclick="deleteSkill(' + skillId + ')">删除</button>' +
                '</div>' +
                '</div>';
        ...
        closeSkillModal();
        showToast(currentSkillId ? '技能已更新' : '技能已添加');
    }
    ...
    function deleteSkill(id) {
        var confirmed = confirm('确定要删除该技能吗？');
        if (confirmed) {
            var card = document.querySelector('[data-skill-id="' + id + '"]');
            if (card) {
                card.remove();
